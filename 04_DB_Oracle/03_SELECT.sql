/*
  ...
  1. DML(Data Manipulation Language) : 데이터 조작어
     - 데이터를 조작하기 위한 언어 (INSERT : 추가, SELECT : 조회, UPDATE : 수정, DELERE : 삭제)
...
/*
   ...
   - 즉, 리터럴은 RESULT SET의 모든 행에 반복적으로 출력
   
   연결 연산자 : ||
   - 여러 컬럼값들을 마치 하나의 컬럼인 것처럼 연결
   - 컬럼값과 리터럴도 연결할 수 있음
*/
...
SELECT *
FROM EMPLOYEE
WHERE 3500000 <= SALARY AND SALARY <= 60000000;
...
-- 부서 배치를 아직 받지 않고(DEPT_CODE) 보너스는 받는 사원들 조회
SELECT *
FROM EMPLOYEE
...
ORDER BY BONUS NULLS LAST; -- 내림차순인 경우는 NULL이 맨앞에
...
-- EMPLOYEE : 성별 사원 수 조회
SELECT
DECODE(SUBSTR(EMP_NO, (INSTR(EMP_NO, '-', 1) + 1), 1), 1, '남', 2, '여'), COUNT(*)
FROM EMPLOYEE
GROUP BY SUBSTR(EMP_NO, (INSTR(EMP_NO, '-', 1) + 1), 1);
