...
BEGIN
   EID := &번호;
   ENAME := '&이름';
   DBMS_OUTPUT.PUT_LINE('ELD : ' || EID);
   DBMS_OUTPUT.PUT_LINE('ENAME : ' || ENAME);
   DBMS_OUTPUT.PUT_LINE('PI : ' || PI);
END;
/

-- 레퍼런스 타입 변수 : 변수명 테이블명.컬러명%TYPE;
...
BEGIN
   ...
   DBMS_OUTPUT.PUT_LIME('입사일 : ' || EMP.HIRE_DATE);
END;
/
/*
   실행부
...
BEGIN
   ...
   IF EMP.BONUS IS NULL 
      THEN DBMS_OUTPUT.PUT_LINE('보너스를 지급받지 않는 사원입니다.');
   ELSE DBMS_OUTPUT.PUT_LINE('보너스는 ' || EMP.BONUS);
   END IF;
END;
/
/*
...
*/
-- 사번을 입력받은 후 부서코드에 따라
-- D1인 경우는 개발팀, D2인 경우는 디자인팀, D3인 경우는 기획팀, 그 외에는 서비스팀
...
BEGIN
   ...
   */
   ...
   DBMS_OUTPUT.PUT_LINE(EMP.EMP_NAME || '는 ' || TEAM || '입니다.');
END;
/
/*
   반복문
...
/*
   ...
   END LOOP;
*/
-- 1~5까지 1씩 증가하는 값을 출력 (역순 -> REVERSE)
BEGIN
