...
VALUES(300, '최다인', 3000000, 36000000); -- 가상 열(연봉)은 사용할 수 없습니다
...
-- 뷰명 : VW_GROUPDEPT
-- 컬럼 : (부서별) 급여 합계, 급여 평균
SELECT DEPT_CODE, SUM(SALARY) "급여 합계", ROUND(AVG(SALARY)) "급여 평균"
FROM EMPLOYEE
GROUP BY DEPT_CODE;
...
VALUES('D1', 1000000, 1000000); -- 그룹 함수들은 이미 계산된 함수식이기 때문에
...
-- 그룹 함수들은 이미 계산된 함수식이기 때문에 모두 다 에러!
...
INSERT INTO VW_JOINEMP VALUES(300, '문채은', '총무부'); -- 에러

INSERT INTO VW_JOINEMP(EMP_ID, EMP_NAME) VALUES(300, '문채은'); -- 성공
...
/*
   ...
   * 데이터 검색 방식
   1. Table Full Scan : 테이블을 처음부터 끝까지 검색하여 원하는 데이터를 찾는 방식
   2. Index Scan : 인덱스롤 통해 데이터를 찾는 방식
*/
-- 현재 인덱스 정보
...
/*
   커서(CURSOR)
   - SQL 쿼리의 결과가 여러 행이 때 처리 결과를 한 행씩 처리하는 객체
...
-- FOR LOOP를 이용한 커서 사용
-- 1. LOOP 시작 시 자동으로 커서 OPEN (즉 OPEN 필요 X)
-- 2. 반복할 때마다 FEETCH도 자동
...
/*
   ...
   FOR EACH ROW
   DECLARE 변수 선언
   BEGIN 실행 내용(지정된 이벤트 발생 시 자동으로 실행할 구문)
   EXCEPTION 예외처리
   END;
   /
...
BEGIN
   ...
   DBMS_OUTPUT.PUT_LINE('변경 전 : ' || :OLD.STOCK);
   DBMS_OUTPUT.PUT_LINE('변경 후 : ' || :NEW.STOCK);
END;
/
INSERT INTO TB_PRODETAIL(PCODE, AMOUNT, STATUS)
VALUES(3, 4, '출고');
