...
-- 직급이 대리인 사원의 사번(EMP_ID), 이름(EMP_NAME) , 직급명(JOB_NAME), 급여 조회(SALARY)
...
*/
-- 사원 사번, 사원명, 사원 부서코드, 사수 사번, 사수명, 사수 부서코드 조회
SELECT EMP_NAME, EMP_NAME, DEPT_CODE, MANAGER_ID FROM EMPLOYEE; -- 사원
SELECT EMP_NAME, EMP_NAME, DEPT_CODE FROM EMPLOYEE; -- 사수

SELECT E.EMP_ID 사번, E.EMP_NAME 사원명, E.DEPT_CODE 사원부서코드, 
       M.EMP_NAME 사수사번, M.EMP_NAME 사수명, M.DEPT_CODE 사수부서코드
FROM EMPLOYEE E
...
JOIN JOB J ON (E.JOB_CODE = J.JOB_CODE); -- USING 사용도 가능!
...
SELECT NATIONAL_NAME 국가명, LOCAL_NAME 지역명, JOB_NAME 직급명, 
       DEPT_TITLE 부서명, EMP_NAME 사원명, SALARY 급여, SAL_LEVEL 급여등급
FROM EMPLOYEE
...
JOIN JOB USING(JOB_CODE)
JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID)
WHERE EMP_NO LIKE '7%' AND SUBSTR(EMP_NO, 8, 1) = 2 AND EMP_NAME LIKE '전%'; -- '7_____-2' 도 가능!
...
SELECT NVL(DEPT_TITLE, '부서 없음') 부서명, TO_CHAR(FLOOR(AVG(SALARY)), '9,999,999') 평균급여
FROM EMPLOYEE
LEFT JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID)
GROUP BY DEPT_TITLE;

-- 5. 각 부서별 총 급여의 합이 1000만원 이상인 부서명, 급여의 합 조회
SELECT DEPT_TITLE 부서명, TO_CHAR(SUM(SALARY), '99,999,999') "급여의 합"
FROM EMPLOYEE
JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID)
GROUP BY DEPT_TITLE
...
FROM EMPLOYEE
JOIN JOB USING(JOB_CODE)
WHERE BONUS IS NULL AND JOB_CODE = 'J4' OR JOB_CODE = 'J7'; -- JOB_CODE IN ('J4', 'J7)로 표현 가능!
...
JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID)
JOIN LOCATION ON (LOCATION_ID = LOCAL_CODE)
WHERE DEPT_CODE IS NOT NULL; -- WHERE 절 생략해도 결과 같음! -> 내부조인에서 NULL 값은 연결되지 않음
...
-- 16. MBTI별 평균 평점 조회
SELECT MBTI, ROUND(AVG(RATING), 2) "평균 평점"
FROM USER_INFO
JOIN REVIEW USING(USER_ID)
GROUP BY MBTI;

-- 17. 각 영화별 리뷰 수와 평균 평점 조회 (리뷰 수가 1건이라도 있는 경우부터)
SELECT TITLE, COUNT(*) "리뷰 수", AVG(RATING) "평균 평점"
FROM REVIEW
JOIN MOVIE USING(MOVIE_ID)
GROUP BY TITLE;

-- 18. 각 배우가 출연한 영화 수 조회 (단, 3편 이상인 배우만)
SELECT NAME 배우, COUNT(*) "영화 수"
FROM MOVIE_ACTOR
JOIN MOVIE USING(MOVIE_ID)
JOIN ACTOR USING(ACTOR_ID)
GROUP BY NAME
