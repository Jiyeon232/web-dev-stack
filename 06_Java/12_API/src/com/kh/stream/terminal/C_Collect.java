...
public class C_Collect {
	...
	public static void main(String[] args) {
		...
		ArrayList<Student> list = students.stream()
							...
							//.collect(Collectors.toList()); // List로 받을 때
							.collect(Collectors.toCollection(() -> new ArrayList<>())); // ArrayList
		System.out.println(list);
		
		// Set 컬렉션으로 수집 -> 고등학생인 사람들만
		HashSet<Student> set = students.stream()
							.filter(student -> student.getAge() < 20)
							//.collect(Collectors.toSet()); // Set으로 받을 때
							.collect(Collectors.toCollection(() -> new HashSet<>())); // HashSet
